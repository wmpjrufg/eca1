...
A [Figura 3](#fig-3) compara os esquemas de vigas de concreto simples, com armadura longitudinal e com armaduras longitudinal e transversal, ilustrando a contribuição das armaduras no comportamento da peça.

(fig-3)=
**Figura 3** – Influência da armadura no comportamento de vigas de concreto: (a) concreto simples; (b) armadura longitudinal de flexão; (c) armaduras longitudinal e transversal (estribos).
...
A [Figura 4](#fig-4) apresenta uma viga de concreto armado em perspectiva, com trechos de concreto omitidos para visualizar as barras longitudinais e os estribos que compõem a armadura.

(fig-4)=
**Figura 4** – Armaduras típicas de uma viga de concreto armado, em perspectiva.
...
- possibilidade de obter estruturas monolíticas e contínuas quando moldado no local.
...
\sigma_c =
\begin{cases}
0{,}85\,f_{cd}\left[1-\left(1-\dfrac{\varepsilon_c}{\varepsilon_{c2}}\right)^{n}\right], & 0\leq\varepsilon_c\leq\varepsilon_{c2} \\[8pt]
0{,}85\,f_{cd}, & \varepsilon_{c2}\leq\varepsilon_c\leq\varepsilon_{cu}
\end{cases}
\tag{7}
$$

em que $f_{cd}$ é a resistência de cálculo à compressão do concreto, $\varepsilon_{c2}$ é a deformação específica no início do patamar plástico e $\varepsilon_{cu}$ é a deformação específica de ruptura do concreto, ambas dependentes da classe de resistência, conforme a [Tabela 2](#tab-2).

(tab-2)=
**Tabela 2** — Valores de $\varepsilon_{c2}$, $\varepsilon_{cu}$ e $n$ em função da classe de resistência do concreto.
...
- **Fluência (ou deformação lenta):** sob carga constante e de longa duração, o concreto continua a se deformar lentamente com o passar do tempo, podendo essa deformação adicional chegar a até quatro vezes o valor da deformação elástica inicial.
...
As barras de aço CA-50 são fornecidas em diâmetros nominais padronizados, com massa linear e área de aço tabeladas em função do diâmetro e da quantidade de barras ([Tabela 3](#tab-3)). Os fios de aço CA-60 seguem a mesma lógica, em diâmetros usualmente menores ([Tabela 4](#tab-4)).

(tab-3)=
**Tabela 3** — Massa linear e área de aço das barras CA-50 em função do diâmetro nominal e da quantidade de barras.
...
(tab-4)=
**Tabela 4** — Massa linear e área de aço dos fios CA-60 em função do diâmetro nominal e da quantidade de fios.
...
O aço também é representado, em projeto, por um diagrama tensão–deformação simplificado: um trecho inicial elástico linear, seguido de um patamar de escoamento a tensão constante. Na ausência de ensaios, adota-se $E_s = 200\text{ GPa}$ para o módulo de elasticidade, e a deformação máxima considerada nas estruturas de concreto armado é limitada a $10\text{‰}$.

> **Figura 6 — Diagrama tensão–deformação simplificado do aço para concreto armado.** Ilustrar o trecho elástico linear (OA) e o patamar plástico de tensão constante (AB), destacando o valor de $f_{yd}$ e a deformação limite de $10\text{‰}$.

```{admonition} Síntese
:class: destaque-azul

O concreto armado só funciona bem porque reúne dois materiais complementares: o concreto absorve a compressão e o aço, a tração, combinação viável graças à aderência entre eles, à compatibilidade de seus coeficientes de dilatação térmica e à proteção que o concreto oferece contra a corrosão da armadura. As propriedades apresentadas nesta aula, tanto do concreto (resistências à compressão e à tração, módulo de deformação, retração e fluência) quanto do aço (resistência ao escoamento e módulo de elasticidade), formam a base sobre a qual os próximos capítulos vão construir o dimensionamento dos elementos estruturais.
```
...
(ref-1)=
**[1]** ASSOCIAÇÃO BRASILEIRA DE NORMAS TÉCNICAS. **NBR 6118**: projeto de estruturas de concreto. 4. ed. Rio de Janeiro: ABNT, 2023.

(ref-2)=
**[2]** ASSOCIAÇÃO BRASILEIRA DE NORMAS TÉCNICAS. **NBR 7480**: aço destinado a armaduras para estruturas de concreto armado. Rio de Janeiro: ABNT, 2024.
